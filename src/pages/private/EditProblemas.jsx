...
const ManageProblemas = () => {
  ...
  const fetchProblemas = async () => {
    try {
      const response = await fetch('http://localhost/RepairIo/php/getAllProblemas.php');
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Estás seguro de que quieres eliminar este problema?')) {
      try {
        const response = await fetch(`http://localhost/RepairIo/php/deleteProblema.php?id=${id}`, {
          method: 'DELETE'
        });
        if (response.ok) {
          fetchProblemas();
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await fetch('http://localhost/RepairIo/php/editProblema.php', {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error('Error updating problema:', data.error || 'Error al actualizar el problema');
    ...
    <div className="container mt-5">
      <div className="d-flex justify-content-between align-items-center mb-4">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b-2 border-black">
                <th className="text-left font-bold py-2">Descripción</th>
                <th className="text-left font-bold py-2">URL App</th>
                <th className="text-left font-bold py-2">Consola</th>
                <th className="text-left font-bold py-2">Acciones</th>
              </tr>
            </thead>
            <tbody>
        ...
        {editing && selectedProblema && (
          <div className="mt-8">
            <h2 className="text-2xl font-bold mb-6 border-b-2 border-black pb-2">Editar Problema</h2>
            <form onSubmit={(e) => { e.preventDefault(); handleUpdate(); }} className="space-y-6">
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setSelectedProblema({ ...selectedProblema, descripcio: e.target.value })}
                  required
                  rows="4"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setSelectedProblema({ ...selectedProblema, URLapp: e.target.value })}
                  required
                />
              </div>
