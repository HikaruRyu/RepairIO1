...
function EditConsola() {
    ...
    const fetchConsolas = async () => {
      try {
        const response = await fetch(
          "http://localhost:8081/getAllConsolas.php"
        );
  ...
  useEffect(() => {
    if (selectedConsola) {
      const fetchConsolaDetails = async () => {
        try {
          const response = await fetch(
            `http://localhost:8081/getConsola.php?idConsola=${selectedConsola}`
          );
          const data = await response.json();
          if (data.status === "success") {
  ...
  useEffect(() => {
    if (selectedConsola && formData.imatge) {
      setImagePath(
        `http://localhost:5173/public/uploads/consolas/${formData.imatge}`
      );
    }
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm("¿Estàs segur que vols eliminar aquesta consola?")) {
      try {
        const response = await fetch(
          "http://localhost:8081/deleteConsola.php",
          {
            method: "POST",
            headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8081/updateConsola.php",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/x-www-form-urlencoded",
